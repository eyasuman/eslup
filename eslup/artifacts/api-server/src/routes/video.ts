...
export function createVideoRouter(
  ...
  const router = Router();
  const getDb = options.getDb ?? getSupabaseAdmin;
  const authenticate = options.authenticate ?? requireParticipant;
  ...
  router.post("/invitations", authenticate, async (req, res) => {
    ...
    if (!UUID.test(appointmentId)) {
      return res
        .status(400)
        .json({ error: "A valid appointment identifier is required" });
    }
    try {
      const db = getDb();
      ...
      if (userId !== patientId)
        return res
          .status(403)
          .json({
            error: "Only the appointment patient can notify the provider",
          });
      if (!doctorId || !appointmentIsEligible(appointment)) {
        return res
          .status(403)
          .json({
            error: "This appointment is not eligible for video consultation",
          });
      }
      ...
      if (existing) {
        if (!invitationMatchesAppointment(existing, appointmentId)) {
          throw new Error(
            "Stored video invitation is missing its appointment link",
          );
        }
      ...
      if (!invitationMatchesAppointment(invitation, appointmentId)) {
        throw new Error(
          "Created video invitation is missing its appointment link",
        );
      }
  ...
  router.post("/sessions", authenticate, async (req, res) => {
    ...
    if (!UUID.test(appointmentId) || !UUID.test(callId)) {
      return res
        ...
        .json({
          error:
            "A valid appointment and accepted call identifier are required",
        });
    }
    try {
      const db = getDb();
      ...
      if (userId !== patientId && userId !== doctorId)
        return res
          .status(403)
          .json({ error: "Not authorized for this appointment" });
      ...
      if (
        callError ||
        !call ||
        call.appointment_id !== appointmentId ||
        asString(call.patient_id) !== patientId ||
        asString(call.doctor_id) !== doctorId ||
        call.status !== "accepted"
      ) {
        return res
          .status(403)
          .json({
            error: "An accepted invitation is required for this appointment",
          });
      }
      ...
      const { data: existing, error: existingError } = await db
        ...
        .eq("appointment_id", appointmentId)
        .eq("status", "active")
        .gt("expires_at", minimumValidUntil)
        .order("created_at", { ascending: false })
        .limit(1)
        .maybeSingle();
      if (existingError) throw existingError;
      ...
      if (!session) {
        await db
          ...
          .eq("appointment_id", appointmentId)
          .eq("status", "active")
          .lte("expires_at", minimumValidUntil);
        const expiresAt = new Date(Date.now() + 10 * 60 * 1000).toISOString();
        const insert = {
          appointment_id: appointmentId,
          call_id: callId,
          room_id: `pulse-${randomBytes(16).toString("hex")}`,
          patient_id: patientId,
          doctor_id: doctorId,
          status: "active",
      ...
      const tokenLifetimeSeconds = Math.max(
        ...
        Math.min(
          600,
          Math.floor((Date.parse(session.expires_at) - Date.now()) / 1000),
        ),
      );
      ...
      return res.json({
        ...
        userId,
        userName: userId === doctorId ? "Provider" : "Patient",
        token,
  ...
  router.post("/sessions/:sessionId/end", authenticate, async (req, res) => {
    ...
    try {
      ...
      if (
        ...
        return res
          .status(403)
          .json({ error: "Not authorized for this video session" });
